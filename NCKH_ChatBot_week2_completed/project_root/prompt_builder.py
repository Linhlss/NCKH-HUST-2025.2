...
def build_style_rewrite_prompt(
    ...
    template = PromptTemplate(
        """
Bạn đang ở bước STYLE REWRITE cuối cùng.
...
[BẢN TRẢ LỜI HIỆN TẠI]
{answer_text}

[YÊU CẦU]
- Viết lại cho rõ ràng, mạch lạc, tự nhiên, chuyên nghiệp.
- Giữ nguyên ý nghĩa và mức độ chắc chắn.
...
""".strip()
    ...
    style_note = (
        "Vì đây là câu trả lời có dùng tài liệu nội bộ, hãy ưu tiên cấu trúc rõ ràng và thừa nhận thiếu dữ liệu nếu có."
        if use_internal_context
        else "Vì đây là câu trả lời general, hãy giữ ngắn gọn và trực tiếp."
    )
    return template.format(
        display_name=profile.display_name,
        persona=profile.persona,
