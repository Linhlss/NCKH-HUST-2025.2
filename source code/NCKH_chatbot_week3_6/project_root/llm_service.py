...
from personalization.paths import tenant_adapter_dir
...
def resolve_personalization_state(profile: TenantProfile) -> Dict[str, object]:
    ...
    return {
        "shared_model_name": profile.shared_model_name or profile.model_name,
...
def get_llm_for_profile(profile: TenantProfile) -> Ollama:
    state = resolve_personalization_state(profile)
    logger.info(
        "LLM runtime selected tenant=%s shared_model=%s adapter=%s mode=%s adapter_available=%s",
        profile.tenant_id,
        state["shared_model_name"],
        state["adapter_name"],
        state["runtime_mode"],
        state["adapter_available"],
    )
    return get_llm(str(state["shared_model_name"]))
