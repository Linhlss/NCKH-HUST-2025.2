...
## Streamlit UI (Week 3 - foundation for LoRA work)

File: `streamlit_app.py`

### Run backend API
```bash
cd NCKH_ChatBot_week3_5/project_root
python api_server.py
```

### Run UI
```bash
cd NCKH_ChatBot_week3_5
streamlit run streamlit_app.py
```

### Current UI modules
- Chat by tenant/user (call `/chat`)
- Runtime status + refresh index + memory reset
- Tenant config editor (`model_name`, `adapter_name`, `top_k`, `persona`, ...)
- LoRA workspace for pipeline1:
  - Upload `.txt` files into `data/files`
  - Run `pipeline1/run_pipeline1.py`
  - Track `generated_qa.json` and `lora_dataset.json`
